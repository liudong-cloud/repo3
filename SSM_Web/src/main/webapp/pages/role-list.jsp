...
<script>
    ...
    function load() {
        $.ajax({
            style: "post",
            url: "<%=request.getContextPath()%>/role/findAll.do",
            ...
            success: function (roleList) {
                ...
                for (var i = 0; i < roleList.length; i++) {
                    ...
                    tt += '<tr>\n' +
                        ...
                        '\t\t\t<a href="javascript:void(0)" onclick="aa(\'' + role.id + '\')" class="btn bg-olive btn-xs1">删除角色</a>\n' +
    ...
    function aa(id) {
        $.ajax({
            ...
            contentType: "application/json",
            dataType: "json",
            success: function (roleList1) {
                var tt = "";
                for (var i = 0; i < roleList1.length; i++) {
                    var role = roleList1[i];
                    tt += '<tr>\n' +
                        '\t\t<td><input name="ids" type="checkbox"></td>\n' +
                        '\t\t<td>' + role.id + ' </td>\n' +
                        '\t\t<td>' + role.roleName + '</td>\n' +
